...
public class KantineAanbod {
     ...
     * String[] artikelnaam
     */ 
    public KantineAanbod(ArrayList<String> artikelnaam, ArrayList<Double> prijs, ArrayList<Integer> hoeveelheid) {
        aanbod=new HashMap<String, ArrayList<Artikel>>();
        for(int i=0;i<artikelnaam.size();i++) 
        {
            ArrayList<Artikel> artikelen=new ArrayList<Artikel>();
            for(int j=0;j<hoeveelheid.get(i);j++) 
            {
                artikelen.add(new Artikel(artikelnaam.get(i), prijs.get(i)));
            }
            aanbod.put(artikelnaam.get(i), artikelen);
        }
    }
